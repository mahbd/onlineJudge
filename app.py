...
def judge_flask(check_id):
    data = json.loads(request.get_json(force=True))
    return judge(check_id, data)


def judge(check_id, data, websocket=None):
    path = os.path.join(DIR, check_id)
    code, input_list, language, output_list, time_limit, correct = get_information(
        data)
    if not correct:
    ...
    if status[0] == 'CE':
        ...
        overall_status = check_test_case(
            path, input_list, output_list, language, time_limit, websocket, check_id)
    if language == 'c_cpp':
        delete_files(path, [''])
    elif language == 'python':
...
def just_output_flask(check_id):
    data = request.get_json(force=True)
    if type(data) != dict:
        data = json.loads(data)
    return just_output(check_id, data)
